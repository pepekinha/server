...
void CMasterUserServer::PrintStats()
{
  ...
  sprintf(buf, "MSINFO: %d (%d max) peers, %d CCU em %d salas, PeakCCU: %d, MaxCCU:%d, Supervisors: %d\n",
    numConnectedPeers_,
    maxConnectedPeers_,
    numCCU,
    numGames,
  ...
  for(size_t i=0; i<supers.size(); ++i)
  {
    ...
    sprintf(buf, "%s(%s), salas:%d/%d, jogadores online:%d/%d\n", 
      super->GetName(),
